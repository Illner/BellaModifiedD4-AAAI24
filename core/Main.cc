                                       ...
                                       vec<bool> &isProjectedVar, bool query, ostream* dratOut)
{
    ...
    TimePointType startTime = std::chrono::steady_clock::now(); // start stopwatch
    DDnnfCompiler<T> *dDnnfCompiler = new DDnnfCompiler<T>(cls, wLit, opt, isProjectedVar, dratOut);
    DAG<T> *t = dDnnfCompiler->compile();
    TimePointType endTime = std::chrono::steady_clock::now();   // stop stopwatch

    long double compilationTime = static_cast<long double>(std::chrono::duration_cast<std::chrono::nanoseconds>(endTime - startTime).count());
    compilationTime /= 1000000000;   // nanoseconds -> seconds

    printf("c Total final time: %Lf\n", compilationTime);
